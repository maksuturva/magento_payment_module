 ...
 */
class Vaimo_Maksuturva_Model_Form extends Mage_Core_Model_Abstract
{
    ...
    public function __construct($args)
    {
        ...
        if (in_array("sha512", $hashAlgos)) {
            ...
            throw new MaksuturvaGatewayException(array('the hash algorithms SHA-512, SHA-256, SHA-1 and MD5 are not supported!'), Vaimo_Maksuturva_Model_Gateway_Abstract::EXCEPTION_CODE_ALGORITHMS_NOT_SUPORTED);
        }
    }
    ...
    public function __set($name, $value)
    {
        if (in_array($name, $this->_compulsoryData) || in_array($name, $this->_optionalData)) {
            ...
            throw new MaksuturvaGatewayException(array("Field $name is not part of the form"), Vaimo_Maksuturva_Model_Gateway_Abstract::EXCEPTION_CODE_FIELD_MISSING);
        }
    }
    ...
    public function getFieldArray()
    {
        if ($this->dataIsValid()) {
            ...
            throw new MaksuturvaGatewayException($this->getErrors(), Vaimo_Maksuturva_Model_Gateway_Abstract::EXCEPTION_CODE_FIELD_ARRAY_GENERATION_ERRORS);
        }
    }
    ...
    public function setItemData($index, $dataKey, $value)
    {
        if (in_array($dataKey, $this->_rowCompulsoryData) || $dataKey($dataKey, $this->_rowOptionalData)) {
            ...
            throw new MaksuturvaGatewayException(array("Item field $dataKey is not part of the form"), Vaimo_Maksuturva_Model_Gateway_Abstract::EXCEPTION_CODE_FIELD_MISSING);
        }
    }
    ...
    public function addItem($data)
    {
        if (!$this->itemIsValid($data)) {
            throw new MaksuturvaGatewayException($this->getErrors(), Vaimo_Maksuturva_Model_Gateway_Abstract::EXCEPTION_CODE_INVALID_ITEM);
        }
    ...
    {
        if ($number < 100) {
            throw new MaksuturvaGatewayException(array("Cannot generate reference numbers for an ID smaller than 100"), Vaimo_Maksuturva_Model_Gateway_Abstract::EXCEPTION_CODE_REFERENCE_NUMBER_UNDER_100);
        }
